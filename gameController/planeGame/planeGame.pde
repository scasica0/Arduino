...
void setup() 
{
    ...
    myPort = new Serial(this, "COM3", 9600); //YOU WILL HAVE TO INSERT YOUR PORT FROM ARDUINO HERE (ARDUINO -> TOOLS ->PORT)
...
void serialEvent(Serial p) { 
    DataIn = p.readString(); 
    int formatData = int(trim(DataIn));
    println(DataIn);
    if (formatData == 1){
      if (IncomingDistance < 85) 
        IncomingDistance += 5;
    }
    if (formatData == 2){
      if (IncomingDistance > 11) 
        IncomingDistance -= 5;
    }
